...
function fixLessonFile(fileName: string) {
  ...
  // Use regex to find and replace holeyCode and correctLines for each exercise
  // We'll extract each exercise block based on "title"
  const exerciseRegex = /\{[^\{\}]*?"title":\s*"[^"]+?"[^\{\}]*?\}/g;
  
  const newContent = content.replace(exerciseRegex, (block) => {
    // Extract correctCode
    ...
    lines.forEach(line => {
      ...
      // If line is already a comment, skip adding another one
      ...
      if (trimmed.startsWith('#include <')) {
        ...
        comment = '// コードを入力';
        holey = line.replace(/([a-zA-Z0-9_]+)/, '___');
...
console.log('Fixed C lesson files with precision.');
