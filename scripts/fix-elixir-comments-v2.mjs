...
for (const file of files) {
  ...
  content = content.replace(regex, (match, p1, correctCode, p3, holeyCode, p5) => {
    ...
    for (let i = 0; i < Math.min(correctLines.length, holeyLines.length); i++) {
      ...
      // If correctLine is a comment (starts with #) and holeyLine is also a comment
      // Copy comment from correct to holey regardless of content
      if (correctLine.trimStart().startsWith('#') && holeyLine.trimStart().startsWith('#')) {
