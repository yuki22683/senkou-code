...
const vaguePatterns = [
  ...
  /^\/\/\s*新しい型の変数を宣言$/,
  /^#\s*新しい型の変数を宣言$/,
  // 追加: 「メソッドを呼び出す」「関数を呼び出す」パターン（具体的なメソッド名/引数がない）
  /^\/\/\s*メソッドを呼び出す$/,
  /^#\s*メソッドを呼び出す$/,
  /^\/\/\s*.*メソッドを呼び出す$/,  // 「〜メソッドを呼び出す」も検出（具体的な引数がない）
  /^#\s*.*メソッドを呼び出す$/,
  /^\/\/\s*関数を呼び出す$/,
  /^#\s*関数を呼び出す$/,
  /^\/\/\s*.*関数を呼び出す$/,  // 「〜関数を呼び出す」も検出
  /^#\s*.*関数を呼び出す$/,
  // 追加: 「関数を実行する」「メソッドを実行」パターン（具体的な関数名/引数がない）
  /^\/\/\s*関数を実行する$/,
  /^#\s*関数を実行する$/,
  /^\/\/\s*.*関数を実行$/,  // 「〜関数を実行」も検出
  /^#\s*.*関数を実行$/,
  /^\/\/\s*.*メソッドを実行$/,  // 「〜メソッドを実行」も検出
  /^#\s*.*メソッドを実行$/,
];
