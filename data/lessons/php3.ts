  ...
  "exercises": [
      ...
      },
    {
      ...
      "tutorialSlides": [
        {
          "title": "Null合体演算子とは？",
          "content": "# null のときの予備\\n\\n**??**（Null合体演算子）は、左側が `null`（空っぽ）のとき **右側の値を使う** 演算子です。\\n\\n**身近な例：**\\n- ニックネームがなければ本名を使う\\n- 予備の電池を用意しておく\\n- 「Aがなければ、Bを使う」\\n\\n**なぜ便利？**\\n- ユーザーが入力しなかったとき → デフォルト値を使う\\n- データがないとき → 初期値を使う\\n- エラーを防げる"
        },
        {
          "title": "?? の使い方",
      ...
      },
    {
      ...
      "tutorialSlides": [
        {
          "title": "スプレッド演算子とは？",
          "content": "# 配列を広げる\\n\\n**スプレッド演算子** `...` は、配列の中身を **バラバラに広げる** 機能です。\\n\\n**身近な例：**\\n- お菓子の袋を開けて中身を出す\\n- 2つの袋のお菓子を1つのお皿に広げる\\n- [あめ、ガム] と [チョコ、クッキー] → [あめ、ガム、チョコ、クッキー]\\n\\n**なぜ便利？**\\n- 複数の配列を簡単に合体できる\\n- 配列のコピーが簡単に作れる"
        },
        {
          "title": "... の使い方",
      ...
      },
    {
      ...
      "correctCode": "// $gradeに'A'を代入\\n$grade = 'A';\\n// match式で$gradeを評価し$messageに結果を代入\\n$message = match($grade) {\\n    // 'A'なら'優秀'を返す\\n    'A' => '優秀',\\n    // 'B'なら'良い'を返す\\n    'B' => '良い',\\n    // それ以外なら'もっと頑張って'を返す\\n    default => 'もっと頑張って'\\n};",
      ...
      },
    {
      ...
      "tutorialSlides": [
        {
          "title": "名前付き引数とは？",
          "content": "# 引数に名前をつけて渡す\\n\\n**名前付き引数** は、「この値はこの引数に入れてね」と **名前を指定** して渡す機能です。PHP 8.0から使えます。\\n\\n**身近な例：**\\n- 注文書に「名前：○○」「住所：○○」と書く\\n- 順番を気にしなくていい\\n- 何を渡しているかが分かりやすい\\n\\n**なぜ便利？**\\n- 引数の順番を覚えなくていい\\n- コードが読みやすくなる\\n- 必要な引数だけ指定できる"
        },
        {
          "title": "名前付き引数の書き方",
      ...
      },
    {
      ...
      "tutorialSlides": [
        {
          "title": "トレイトとは？",
          "content": "# コードの共有パーツ\\n\\n**トレイト** は、複数のクラスで **同じメソッドを共有** する仕組みです。\\n\\n**身近な例：**\\n- レゴブロックのパーツ\\n- 「車輪パーツ」を車にもバスにもつけられる\\n- 同じ機能を色んなクラスで使い回せる\\n\\n**継承との違い：**\\n- 継承：1つの親からしか機能をもらえない\\n- トレイト：複数のトレイトを組み合わせられる\\n- 「いいとこ取り」ができる！"
        },
        {
          "title": "トレイトの書き方",
      ...
      },
    {
      ...
      "tutorialSlides": [
        {
          "title": "ジェネレータとは？",
          "content": "# 値を1つずつ出す関数\\n\\n**ジェネレータ** は、値を **一度に全部ではなく1つずつ** 生成する特別な関数です。\\n\\n**身近な例：**\\n- ガチャガチャ（カプセルトイ）のマシン\\n- ハンドルを回すたびに1つずつカプセルが出てくる\\n- 全部を一度に出さない → メモリを節約できる\\n\\n**なぜ便利？**\\n- 100万個のデータも、1つずつ処理すればメモリを使いすぎない\\n- 大きなファイルを1行ずつ読むときに便利\\n- 無限に続く値も扱える"
        },
        {
          "title": "yield の使い方",
      ...
      },
    {
      ...
      "tutorialSlides": [
        {
          "title": "分割代入とは？",
          "content": "# 配列をバラして変数に入れる\\n\\n**分割代入** は、配列の中身を **一度に複数の変数に入れる** 機能です。\\n\\n**身近な例：**\\n- プレゼントの箱を開けて、中身を別々の場所に置く\\n- [名前、年齢、住所] → 名前変数、年齢変数、住所変数に分ける\\n- 1個ずつ取り出すより早い！\\n\\n**なぜ便利？**\\n- 1行で複数の変数に代入できる\\n- コードがスッキリする\\n- 関数から複数の値を受け取るときに便利"
        },
        {
          "title": "分割代入の書き方",
          "content": "# [ ] で変数を並べる\\n\\n`[$a, $b, $c] = 配列;` の形で書きます。\\n\\n**コード例：**\\n```php\\n$arr = [1, 2, 3];\\n[$a, $b, $c] = $arr;\\necho $a;  // 1\\necho $b;  // 2\\necho $c;  // 3\\n```\\n\\n**何をしているの？**\\n1. `$arr = [1, 2, 3]` → 配列を用意\\n2. `[$a, $b, $c] = $arr` → 順番に変数へ入れる\\n3. `$a` に 1、`$b` に 2、`$c` に 3 が入る\\n\\n**普通の書き方との比較：**\\n```php\\n// 普通（長い）\\n$a = $arr[0];\\n$b = $arr[1];\\n$c = $arr[2];\\n\\n// 分割代入（短い）\\n[$a, $b, $c] = $arr;\\n```\\n\\n**ポイント：**\\n- 左側の変数の数と配列の要素数を合わせる\\n- 順番通りに代入される"
        }
      ],
