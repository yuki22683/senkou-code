...
### 75. 警告の厳格遵守 (Strict Compliance with Warnings)

- `check-lesson-file.mjs` などのチェックスクリプトが出力する警告（Warning）を、理由なく放置してはならない。

- 警告は「エラーではないが潜在的なルール違反」を示している。特にルール #31（連続コメント禁止）のような重要なルールは、警告からエラー（Error）に格上げして強制力を高めること。

- 作業完了を宣言する前に、全ての警告が解決されているか、あるいは妥当な理由（技術用語の誤検知等）があるかを確認すること。



### 76. 監査ロジックの堅牢性確保 (Audit Logic Robustness)

- レッスンデータの整合性をチェックするスクリプトを作成・使用する際は、単純な文字列分割（`split(',')`）や正規表現を避け、文字列内のカンマやエスケープ文字を正しく処理できる堅牢なパースロジック（例: ルール #67）を採用すること。

- 不十分なパースロジックは、実際には違反が存在するにもかかわらず「問題なし」と誤判定する原因となり、プロジェクトの品質を低下させる。

- 監査ツール自体が正しいことを、意図的な違反データ（ネガティブテスト）を用いて検証すること。
