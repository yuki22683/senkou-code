...
### 93. 空行をholeyCodeで`___`にしない
- holeyCodeで空行（correctLinesが`""`の行）を`___`に置き換えてはならない。空行は空行のまま残すこと。
- **禁止パターン**:
  - correctCode: `...brand = brand\\n\\n# classで...`（空行あり）
  - holeyCode: `...___ = ___\\n___\\n# classで...`（空行が`___`になっている）
- **正しいパターン**:
  - holeyCode: `...___ = ___\\n\\n# classで...`（空行は空行のまま）
- **理由**: `___`だけの行は虫食い行として判定され、エディタがその行にカーソルを移動してしまう。空行は編集対象ではないので、`___`にしてはならない。
- **チェック方法**: holeyCodeとcorrectLinesを比較し、correctLinesが`""`の行に対応するholeyCodeの行も空であることを確認する
