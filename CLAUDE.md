...
- **禁止表現**：
  ...
  - **`# 関数を呼び出す`** → 何の関数か不明（`# hello関数を'Python'で呼び出す` のように関数名と引数を含める）
  - **`// 関数を呼び出す`** → 関数名と引数が不明（`// check関数（引数10）を呼び出す` のように）
  - **`// 〜を呼び出す`**（引数省略）→ 引数が不明（`// repeat関数（引数3）を呼び出す` のように引数も明記）
  - **`// メソッドを呼び出す`** → 何のメソッドか不明（`// Greetメソッドを呼び出す` のようにメソッド名を含める）
  ...
  - **`# リストを定義`** → 何のリストか不明（`# 数値配列（1〜5）をnumbersに代入` のように）
  - **`# 辞書データを定義`** → 何の辞書か不明（`# dataにlanguage='Python', level='advanced'の辞書を代入` のように）
  - **`# 文字列を定義`** → 何の文字列か不明（`# textに'hello world'を代入` のように）
  - **`# 配列を定義`** → 何の配列か不明（`# 色配列（赤、青、緑）をcolorsに代入` のように）
  - **`# 変数を定義`** → 何の変数か不明（`# scoreに100を代入` のように）
  - **`# 値を代入`** → 何を何に代入するか不明（`# $textに改行付き文字列を代入` のように変数名と内容を明記）
  - **`// 値を代入`** → 同上（`// 構造体変数を初期化`、`// ファイルを書き込みモードで開く` のように具体的に）
  - **`// 変数を宣言`** → 何の変数か不明（`// scoreに点数を代入` のように変数名と内容を明記）
  - **`# 配列を作成`** / **`// 配列を作成`** → 同上
  - **`# リストを作成`** / **`# 辞書を作成`** / **`# ハッシュを作成`** → 同上
  - **`# 〜という配列を作る`** → 何を入れるか不明（`# 色配列（赤、青）をcolorsに代入` のように）
  - **`# 変数を表示`** → 何の変数か不明（`# scoreを表示` のように変数名を含める）
  - **`# 〜を変換`** → 何を何に変換して何に代入するか不明（`# json_strを辞書に変換してdataに代入` のように入力・出力・代入先を明記）
  - **`# mapで各要素を変換`** → 何の配列を何に変換するか不明（`# numsの各要素を2倍にしてdoubledに代入` のように）
  - **`# 文字列を数値に変換`** → 何の文字列を何に代入するか不明（`# numに文字列'abc'をint変換して代入` のように）
  - **`// nameという名前のはこを作る`** → 何を代入するか不明（`// nameに'JavaScript'を代入` のように）
  - **`// 〜ラベルを貼ったはこを作る`** → 何を代入するか不明（`// Userオブジェクト（名前:たろう）をuserに代入` のように）
  - **`// vector で配列を作成`** → 何を入れるか不明（`// 数値配列（10、20、30）をnumsに代入` のように）
  - **`// int で変数を宣言`** → 何の値か不明（`// xに5を代入` のように）
  - **`// asList でリストを作成する`** → 何を入れるか不明（`// 数値配列（1、2、3）をnumsに代入` のように）
  ...
  - **`# 関数（第1引数に小さい方, 第2引数に大きい方）を呼び出す`** → 実際の値が不明（`# add関数（第1引数に10, 第2引数に20）を呼び出す` のように具体的な値を書く）
  ...
  - **ループ展開のコメントには関数名を含める**: `for i, v in enumerate(lst)` や `for k, v in dict.items()` のような行では、使用する関数/メソッド名を必ずコメントに含めること。
    - ❌ `# 1番目の i に番号、2番目の v に要素が入る` → 関数名がない
    - ✅ `# lstからenumerate()でインデックスi・要素vを取得しループ` → 関数名を含む
    - ❌ `# 1番目の k にキー、2番目の v に値が入る` → 関数名がない
    - ✅ `# dictからitems()でキーk・値vを取得しループ` → メソッド名を含む
  - **コードをそのままコメントにしない**: `// c.Compare(5, 3)を表示`、`# Dog.get_count()を表示`、`// nums.reduce(1)で計算` のようにコードをそのまま書くのは禁止。コードの**意味**を日本語で書くこと。
    - ❌ `# 関数をx=10, y=20で呼び出す` → キーワード引数がコード形式
    - ✅ `# 関数をキーワード引数（xに10、yに20）で呼び出す` → 日本語形式で説明
    - ❌ `// c.Compare(5, 3)を表示` → コードそのまま
    - ✅ `// 5と3を比較した結果を表示` → 意味を説明
    - ❌ `# Dog.get_count()を表示` → コードそのまま
    - ✅ `# 生成されたDogの数を表示` → 意味を説明
    - ❌ `// a * bを返す` → コードそのまま
    - ✅ `// 2つの数を掛けた結果を返す` → 意味を説明
    - ❌ `# colorsに['あか', 'あお']を代入` → 配列リテラルがコードそのまま
    - ✅ `# 色配列（あか、あお）をcolorsに代入` → 括弧形式で内容を説明
    - ❌ `// numsに{10, 20, 30}を代入` → 初期化子リストがコードそのまま
    - ✅ `// 数値配列（10、20、30）をnumsに代入` → 括弧形式で内容を説明
    - ❌ `# colors[1]を表示` → 配列アクセスがコードそのまま
    - ✅ `# 2番目の色（あお）を表示` → 何番目・何の値かを説明
    - ❌ `# greet()を呼び出す` → 関数呼び出しがコードそのまま
    - ✅ `# あいさつ関数を呼び出す` → 関数の意味を説明
    - ❌ `// check(5)の戻り値を代入` → 引数付き関数呼び出しがコードそのまま
    - ✅ `// check関数（引数5）の戻り値を代入` → 関数名（引数X）形式で説明
    - ❌ `// new Car()を代入` → インスタンス作成がコードそのまま
    - ✅ `// 新しいCarインスタンスを代入` → 日本語で説明
    - ❌ `// Some(n)ならnを出力` → パターンマッチがコードそのまま
    - ✅ `// Someパターン（変数n）ならnを出力` → パターン（変数X）形式で説明
    - ❌ `// user.getPassword()の結果を表示` → メソッド呼び出しがコードそのまま
    - ✅ `// userのgetPasswordメソッドの結果を表示` → 「のXメソッド」形式で説明
- **正しい表現**：
...
- **コード形式コメントの手動チェックコマンド**：
  ```bash
  # 配列アクセス形式: 変数[キー]を
  grep -E '"(#|//|--) .*\[[^\]]+\](を|の値)' data/lessons/*.ts | grep -v tutorialSlides | grep -v content | grep -v 'の2番目' | grep -v 'のキー' | grep -v 'のインデックス'

  # 関数呼び出し形式: func(数字)を
  grep -E '"(#|//|--) .*[a-zA-Z_]+\([0-9]+\)(を|の結果|で)' data/lessons/*.ts | grep -v tutorialSlides | grep -v content | grep -v '関数（' | grep -v 'メソッド（'

  # new インスタンス形式: new Class()を
  grep -E '"(#|//|--) .*new [A-Z][a-zA-Z]*\([^)]*\)(を|に)' data/lessons/*.ts | grep -v '新しい' | grep -v 'インスタンス'
  ```
  全て0件であることを確認すること。
...
- **色の表記はひらがなで統一**：
  ...
  - コメント内も同様（`# 色の配列（あか、あお）をcolorsに代入`）
  - 理由：漢字とひらがなが混在すると統一感がなくなる
- **名前の表記もひらがなで統一**：
  - `"太郎"` → `"たろう"`、`"花子"` → `"はなこ"`
  - コード内が `'たろう'` なら、コメント内も「たろう」にする
  - ❌ `# 名前のリスト（太郎、花子）をnamesに代入` + `names = ['たろう', 'はなこ']` → コメントとコードが不一致
  - ✅ `# 名前のリスト（たろう、はなこ）をnamesに代入` + `names = ['たろう', 'はなこ']` → 一致している
- **出力文字列の漢字もひらがな/カタカナで統一**：
  - `"合格"` → `"ごうかく"`、`"不合格"` → `"ふごうかく"`
  - `"大人"` → `"おとな"`、`"子供"` → `"こども"`、`"未成年"` → `"みせいねん"`
  - `"偶数"` → `"ぐうすう"`、`"奇数"` → `"きすう"`
  - `"秘密"` → `"ひみつ"`
  - 理由：初学者向けのため、漢字よりひらがな/カタカナの方が読みやすい
- **チェックコマンド**：
  ```bash
...
- **正しい**：
  - `print('ごうかく！')` → 文字列がひらがな
  - `console.log('おとな')` → 文字列がひらがな
  - `// nameに'たろう'を設定` → コメント内の参照もひらがな
- **理由**：
  - 日本語学習者向けのため、文字列も日本語の方が理解しやすい
  - コメントと文字列の両方が日本語で統一されていると混乱が少ない
- **修正スクリプト**：
  - `node scripts/convert-kanji-to-hiragana.mjs` - 漢字をひらがなに変換
- **チェックスクリプト**：
  - `node scripts/check-kanji-in-lessons.mjs` - 残りの漢字を検出
...
### 78. 複数引数/複数要素は全て虫食いにし、コメントで順番を一意に特定できるようにする
- 関数呼び出し、コンストラクタ、配列/リストで**複数の値がある場合、全ての値を虫食い（`___`）にする**こと（ルール#67との整合性）。
- コメントで**どの値がどの位置に入るか一意に特定できるように記述**すること。
- **禁止パターン（一部だけ虫食い）**:
  - `Rectangle(___, 5)` → 5がそのまま残っている
  - `add(___, 20)` → 20がそのまま残っている
  - `Team(['アリス', ___, 'チャーリー'])` → 一部だけ虫食い
- **禁止パターン（コメントが曖昧 - 順番が一意に特定できない）**:
  - `# Rectangleを作成（第1引数=幅, 第2引数=高さ）` → 4と5のどちらが幅か不明
  - `# add関数（第1引数, 第2引数）を呼び出す` → 10と20のどちらが第1引数か不明
  - `# 3人のチームをteamに代入` → メンバーの順番が不明
  - `# Studentを作成（名前に文字列, 学年に数値）` → 候補に文字列が複数あると特定不可
- **正しいパターン（具体的な値で順番を明示）**:
  - `# チーム（アリス、ボブ、チャーリーの順）をteamに代入` → 順番が明確
  - `# Studentを作成（名前に'太郎', 学年に3）しsに代入` → 具体的な値で一意に特定
  - `# Personを作成（名前に'太郎', 年齢に25）しpに代入` → 具体的な値で一意に特定
  - `# Pointインスタンスを作成（X=3, Y=4）` → 具体的な値で一意に特定
  - `# 数値配列（10、20、30の順）をnumsに代入` → 順番が明確
- **特徴で一意に特定できる場合のみ特徴を使用可**:
  - 型が完全に異なる: `# 名前（文字列）と年齢（数値）でPersonを作成` → 候補に文字列が1つ、数値が1つしかない場合のみOK
  - 数値の大小が明確: `# Rectangleを作成（幅に小さい方4, 高さに大きい方5）` → 値も併記
...
- **理由**: 一部だけ虫食いだと、ユーザーが「その値は入力不要」と誤解する。コメントで順番が一意に特定できないと、ユーザーは当てずっぽうで入力するしかなくなる
- **関数定義の場合**: 関数定義で複数の引数がある場合は、**コメントで引数名と順番を明示**すること。
  - ❌ `# 2つの数を足して返すadd関数` → 引数の順番が不明
  - ✅ `# add関数を定義（第1引数a, 第2引数b）` → 引数名と順番が明確
...
### 84. 演習にはdescriptionフィールドを必ず設定する
- 全ての演習には**descriptionフィールド**を設定し、演習を一言で紹介する文を記述すること。
- **形式**: 「〜を学びます」「〜の使い方を学びます」など、演習内容を簡潔に説明する文
- **例**:
  - タイトル「画面に文字を出してみましょう」→ description「画面に文字を表示する方法を学びます」
  - タイトル「リスト内包表記」→ description「1行でリストを作成する便利な書き方を学びます」
  - タイトル「変数（へんすう）」→ description「データを保存する「変数」の使い方を学びます」
- **禁止パターン**:
  - descriptionを省略してnullにする → 演習一覧で詳細文が表示されなくなる
  - タイトルをそのまま使う「〜しましょうの仕組みと使い方を学びます」→ 意味不明な文になる
  - 単純すぎる「〜の使い方を学びます」だけ → タイトルの繰り返しで情報量がない
- **良い説明文の条件**:
  - 演習で何ができるようになるか具体的に説明する
  - 専門用語には補足（カッコ書き）を入れる
  - 学習者が「やってみたい」と思える内容にする
- **新規演習追加時**: 必ずdescriptionフィールドを追加すること
- **改善スクリプト**: `node scripts/improve-exercise-descriptions.mjs`（より丁寧な紹介文に改善）

### 85. tutorialSlidesのコード出力表示形式を統一する
- `tutorialSlides`のコード例で出力結果を示す際は、**コードブロック外**に `=> 値` の形式で表示すること。
- **禁止パターン（コード内コメント）**:
  ```python
  print(10 % 3)  # => 1
  ```
- **正しいパターン（コードブロック外）**:
  ...
  => 1
- **理由**: コード内コメントと出力結果が混在すると、学習者がどれがコードでどれが出力かわかりにくい
- **修正スクリプト**: `node scripts/unify-output-format.mjs`（v1〜v5で段階的に処理）

### 86. holeyCodeを\nで分割するスクリプトは文字列リテラル内の\nに注意
- holeyCodeを `\n` リテラル（`String.fromCharCode(92, 110)`）で分割するスクリプトでは、**文字列リテラル内の `\n` も分割されてしまう**問題に注意すること。
- **問題のパターン**:
  - holeyCode: `___.___(\"___: ___\\n\", ___, ___)` （printf文など）
  - ファイル上の `\\n` は3バイト（`\`, `\`, `n`）
  - `\n` リテラル（2バイト）で分割すると、`\`, `\`, `n` の中間でマッチして分割される
- **症状**: holeyLinesの行数がcorrectLinesより多くなり、配列長不一致でスキップされる
- **対処法**:
  1. **演習データを修正**: 文字列リテラル内の `\n` を避ける
     - `fmt.Printf("%s: %d\n", k, v)` → `fmt.Println(k, v)` に変更
     - `print "$data\n"` → コメントを追加して行数を合わせる
  2. **スクリプトを改良**: 文字列リテラル内の `\n` を無視するパーサーを実装（複雑）
- **影響を受けたファイル**: go3.ts, perl3.ts（手動で修正済み）
- **チェックスクリプト**: `node scripts/count-missing-comments.mjs`（配列長不一致の演習はスキップされカウントされない）
- **再発防止**: 新規演習追加時、printf系の文字列に `\n` を含める場合は、correctLinesとの整合性を確認すること

### 87. 出力コメントには具体的な出力対象を明記する
- `# 出力`、`// 出力` のような曖昧なコメントは**禁止**。
- 出力する変数名や値を必ず含めること。
- **禁止パターン**:
  - `# 出力` → 何を出力するか不明
  - `// 出力` → 何を出力するか不明
- **正しいパターン**:
  ...
  - `# $fruits[0]を出力`
  - `// todayを出力`
- **チェックコマンド**: `grep -E '"(#|//) 出力"[,\]]' data/lessons/*.ts`
  - 0件であることを確認する
...
### 89. 関数呼び出しコメントの日本語文法
- 「〜関数を呼び出し（引数）して〜」は**不自然な日本語**（「呼び出しして」になる）。
- **禁止パターン**:
  - `# add関数を呼び出し（第1引数に10, 第2引数に20）してresultに代入`
  - `// minmax関数を呼び出し（第1引数に5, 第2引数に3）してmin, maxに代入`
- **正しいパターン**:
  - `# add関数（第1引数に10, 第2引数に20）を呼び出してresultに代入`
  - `// minmax関数（第1引数に5, 第2引数に3）を呼び出してmin, maxに代入`
- **修正パターン**: `〜関数を呼び出し（引数）して〜` → `〜関数（引数）を呼び出して〜`
- **チェックコマンド**: `grep -r "を呼び出し（.*）して" data/lessons/*.ts`

### 90. ループ変数宣言をコメント冒頭に書かない
- コメントの冒頭に「for 変数, 変数で」のようなループ構文を書かない
- **禁止パターン**:
  - `# for k, vでkwargsからitems()で...` → 「for k, v」がコード構文
  - `# for i, colorでcolorsからenumerate()で...` → 「for i, color」がコード構文
  - `# for numでeven_numbers関数の結果を...` → 「for num」がコード構文
- **正しいパターン**:
  - `# kwargsからitems()でキーkと値vを取得してループ`
  - `# colorsからenumerate()でインデックスiと要素colorを取得しループ`
  - `# even_numbers関数（引数4）の各結果をnumに代入してループ`
- **ポイント**: ループの対象（コレクション/関数）を主語にし、取得する変数を説明に含める
- **チェックコマンド**: `grep -E '"# for [a-z]+[,\s]' data/lessons/*.ts`

### 91. 変数埋め込み文字列のコメントは出力形式を明示する
- 「f文字列」等の専門用語を使わず、埋め込む変数と出力形式を明示する
- **禁止パターン**:
  - `# print関数でf文字列「{k} = {v}」を表示` → 「f文字列」が意味不明
  - `# print関数で「k = v」の形式で出力` → 変数埋め込みの情報が欠落
  - `# print関数でkとvを埋め込んで「k = v」形式で出力` → 出力形式が不明確
- **正しいパターン**:
...
### 92. 編集対象行の判定でコメント行・空行を除外する
- `isEditableLine` 関数など「編集対象行かどうか」を判定するロジックでは、**その行自体がコメント行や空行でないこと**を最初にチェックすること。
- **禁止パターン**:
  - 「コメントの次の行は編集対象」という判定で、その行自体がコメント行かどうかを確認しない
  - 結果: 連続するコメント行の2行目が編集対象と誤判定され、カーソルがコメント行に移動してしまう
- **正しいパターン**:
  1. まずその行自体がコメント行（`#`, `//` 等で始まる）なら `false` を返す
  2. 空行なら `false` を返す
  3. その後で「虫食い行」「コメントの次の行」などの判定を行う
- **理由**: 「コメントの次の行」が必ずコード行とは限らない（連続するコメント行がある場合）

### 93. 空行/インデントのみの行をholeyCodeで`___`にしない
- holeyCodeで空行（correctLinesが`""`の行）やインデントのみの行（correctLinesが`"    "`など空白のみの行）を`___`に置き換えてはならない。correctLinesと同じ内容を維持すること。
- **禁止パターン**:
  - correctLines: `""`（空行）→ holeyCode: `___`（空行が`___`になっている）
  - correctLines: `"    "`（インデント4つ）→ holeyCode: `    ___`（インデント+`___`になっている）
- **正しいパターン**:
  - correctLines: `""`（空行）→ holeyCode: ``（空行のまま）
  - correctLines: `"    "`（インデント4つ）→ holeyCode: `    `（インデントのみ）
- **理由**: `___`を含む行は虫食い行として判定され、エディタがその行にカーソルを移動してしまう。空行やインデントのみの行は編集対象ではないので、`___`にしてはならない。
- **修正スクリプト**: `node scripts/fix-holey-empty-lines.mjs`
- **チェック方法**: holeyCodeとcorrectLinesを比較し、correctLinesが空行/インデントのみの行に対応するholeyCodeの行も同じ内容であることを確認する

### 94. DB内の演習重複を定期チェックする
- `seed:db` は演習の追加/更新のみで削除しないため、過去に別のorderIndexで登録された演習がDBに残り続け、重複が発生する
- **定期チェック**: `npx ts-node scripts/check-db-duplicates.ts` を実行し、重複がないことを確認
- **重複が見つかった場合**:
  1. `npx ts-node scripts/delete-exercise.ts "演習タイトル"` で削除
...
### 102. コメントにコードをそのまま書かない（変数名は保持）
- コメントにコード構文（`変数['キー']`、`変数.メソッド('値')`等）をそのまま書いてはならない。
- ただし**変数名は保持**すること（どの変数を操作するかわかるようにする）。
- **禁止パターン**:
  - `# d['くだもの']に'りんご'をappend` → `d['くだもの']`がコード構文
  - `# data['name']を表示` → `data['name']`がコード構文
  - `# list.append('値')で追加` → `list.append('値')`がコード構文
- **正しいパターン**:
  - `# dのくだものキーにりんごを追加` → 変数名`d`は保持、構文は日本語化
  - `# dataのnameキーを表示` → 変数名`data`は保持
  - `# listにappendで値を追加` → 変数名`list`は保持、メソッド名は説明的に
- **チェックコマンド**:
  ```bash
  grep -rn "\"# .*\['[^']*'\]" data/lessons/*.ts | grep -v tutorialSlides | grep -v '"content"'
  ```
- **水平展開済み**: 全76ファイル調査完了（問題はpython5.tsのdefaultdict演習のみ、修正済み）
